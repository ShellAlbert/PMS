...
bool StdStorageDialog::populate()
{
    ...
    foreach(const StorageObjectInfo & info, fileInfoList) {
        ...
        objectItem->setFlags(Qt::ItemFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable) /*| Qt::ItemIsEditable*/);
        objectItem->setData(0, urlRole, info.url);

        if (i.fileName() == m_proposedFileName)
...
void StdStorageDialog::dirUp()
{
    ...
    m_currentPath = m_currentPath.section("/", 0, -3) + "/";
