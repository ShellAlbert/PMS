...
void OptionsDialogRendererPage::listIndexChanged(QTreeWidgetItem* current, QTreeWidgetItem* previous)
{
    Q_UNUSED(previous)
    if (!current) {
    ...
    if (previous) {
        ...
        CuteReport::RendererInterface * object = m_core->reportCore()->renderer(objectName);
        if (object)
    ...
    if (m_currentHelper) {
        CuteReport::RendererHelperInterface * helper = qobject_cast<CuteReport::RendererHelperInterface*>(m_currentHelper.data());
        if (helper)
            helper->save();
    }
    delete m_currentHelper;
...
void OptionsDialogRendererPage::setNewObject()
{
    QAction * action = qobject_cast<QAction*>(sender());
    Q_ASSERT(action);
    QString moduleName = action->data().toString();
    CuteReport::RendererInterface * object = m_core->reportCore()->createRendererObject(moduleName);
    if (!object)
        return;
    object->setObjectName(CuteReport::ReportCore::uniqueName(object, object->objectNameHint(), m_core->reportCore()));
    m_core->reportCore()->setRenderer(object);

    updateObjectList();
}


void OptionsDialogRendererPage::deleteCurrent()
{
    if (!ui->objectList->currentItem())
        return;
    QString name = ui->objectList->currentItem()->text(0);
    m_core->reportCore()->deleteRenderer(name);
    updateObjectList();
}


void OptionsDialogRendererPage::setDefault()
{
    if (!ui->objectList->currentItem())
        return;
    QString name = ui->objectList->currentItem()->text(0);
    m_core->reportCore()->setDefaultRenderer(name);
    ui->leDefault->setText(m_core->reportCore()->defaultRendererName());
}


void OptionsDialogRendererPage::clearDefault()
{
    m_core->reportCore()->setDefaultRenderer(QString());
    ui->leDefault->setText(m_core->reportCore()->defaultRendererName());
}


void OptionsDialogRendererPage::updateObjectList()
{
    ui->objectList->clear();
    QStringList nameList = m_core->reportCore()->rendererNameList();
    qSort(nameList);
...
void OptionsDialogRendererPage::slotObjectNameChanged(const QString &name)
{
