...
Core::Core(QWidget *winParent,QObject *parent) :
    QObject(parent),
    m_mainWindow(0),
    m_currentReport(0),
    m_currentPage(0),
...
{
    m_mainWindow = new MainWindow(this->m_winParent,this);
    m_mainWindow->setWindowTitle("PMS Plus报表设计器");
...
void Core::slotMainWindowCloseRequest()
{
    ...
    emit closed();
    emit this->ZSignalCloseEvent("CuteReport");
}
